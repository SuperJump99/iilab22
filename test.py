    ...
    for key, value in remain_SCHEDULE_dict.items():
        ...
        if value:
            ...
            if int(operation.split('_')[2]) - 1 == job_current_finished_operation_index_list[lot_num-1]:
                ...
                gantt_data.append([key, operation_cut, max_value + setup_time, max_value + setup_time + processing_time, value[0]])
                Job.append(job)
                Lots.append(value[0])
...
class Gantt:
    def __init__(self, gantt_data, Job, Lots):
    ...
    def Save_gantt_data_to_xlsx(self, gantt_data):
        # 간트 데이터 엑셀로 저장
    ...
    def df_Gantt_Chart(self):
        ...
        print(gantt_data)
        print(self.Color())
        fig = create_gantt(gantt_data, index_col='Color', bar_width=0.15, show_colorbar=True, colors = self.Color(),
    ...
    def xlsx_Gantt_Chart(self):
        ...
        fig = create_gantt(df, index_col='Color', bar_width=0.15, show_colorbar=True, colors=self.Color(Job),
                           showgrid_x=True, showgrid_y=False, title="Time Line",
                           show_hover_fill=True, group_tasks=True, )
        fig.layout.xaxis.type = "linear"
        return fig.show()

first = Gantt(gantt_data,Job, Lots)
print(first.Color())
first.df_Gantt_Chart()
