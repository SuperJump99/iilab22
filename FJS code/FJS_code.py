    ...
    for key, value in remain_SCHEDULE_dict.items():
        if value:
            ...
            if int(operation.split('_')[2]) - 1 == job_current_finished_operation_index_list[lot_num - 1]:
                ...
                max_value = max(job_completion_time_list[JOBTYPE_JOB[operation.split('-')[1][0]]],
                                machine_completion_time_list[int(key.split('_')[1]) - 1])
                job_completion_time_list[JOBTYPE_JOB[operation.split('-')[1][0]]] = max_value + PROCESSING_TIME[
                    key + '-' + operation.split('-')[1]]  # 끝나는 시간 = 시작시간 + 생산시간
                machine_completion_time_list[int(key.split('_')[1]) - 1] = max_value + PROCESSING_TIME[
                ...
                if setup.split('_')[0] == operation_cut.split('_')[0]:
                    if setup.split('_')[1] == operation_cut.split('_')[1]:
                        ...
                        job_completion_time_list[JOBTYPE_JOB[operation_cut.split('_')[0]]] += SETUP_TIME[
                            'homogeneous_setup']
                ...
                print("PROCESSING", key, operation.split("-")[1], max_value + setup_time,
                      max_value + setup_time + processing_time)
