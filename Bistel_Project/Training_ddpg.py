...
batch_size = 128    # 업데이트 묶음 단위
mem_maxlen = 30000  # 리플레이 버퍼에 들어가는 메모리 최대값
discount_factor = 0.9
actor_lr = 1e-4
critic_lr = 5e-4
tau = 1e-3
...
# 모델 저장 및 불러오기 경로
date_time = datetime.datetime.now().strftime("%Y%_m%_d%_H%M%S")
...
class DDPGAgent():
    ...
    def train_model(self):
        ...
        # Critic 업데이트
        next_actions = self.target_actor(next_state)        # 상태의 가치를 평가
        ...
        # Actor 업데이트
        action_pred = self.actor(state)         # 상태를 줬을때 행동을 결정
...
if __name__ == '__main__':
    ...
    for input in range(num_of_inputs):          # TODO: input-> 각 state의 초기 상태의 개수
        for step in range(run_step):           # TODO: total calculation = num_of_inputs * test_step


            # TODO: state action reward 수정 필요
            state = dec.obs[0]
            ...
            agent.append_sample(state[0], action[0], reward, next_state[0], [done])    # 리플레이 버퍼에 추가

            if step > max(batch_size, train_start_step):    # 리플레이 버퍼에 일정량이 쌓이게 되면 학습 시작
                # 학습 수행
